...
function InputListBox({ label, onClick }) {
  ...
  return (
    <li
      className={`form_line_list_box ${onClick ? "clickable" : ""}`}
      onClick={click}
    >
      {label}
    </li>
  ...
  return (
    <div className="form_line">
      ...
      <div className="form_line_input_frame">
        ...
        />
        <button
          className="form_line_input_search"
          type="button"
          onClick={click}
        >
          검색
      ...
      <ul className={`form_line_list ${list.length > 0 ? "active" : ""}`}>
        {list.map((li) => (
          <InputListBox label={li.label} onClick={li.click} />
        ))}
      </ul>
      <div className="form_line_error">{error}</div>
    </div>
  ...
  return (
    <div className="form_frame">
      ...
      <p className="form_desc">
        학급 정보를 입력한 뒤, 창의경영고 일학습병행 홍보 영상 '1분 1초'를 30초
        이상 시청해주세요.
      </p>
      <form>
        <InputLine
